...
import { checkAuthenticationStatus, disableAuthenticationStatus, enableAuthentication, getGoogleAuthenticatorQR, sendVerification } from "../../store";
/**
...
const AuthTypes = ({
  ...
  useEffect(() => {
    if (isFocused) {
      ...
      dispatch(checkAuthenticationStatus()).then(unwrapResult).then(res => {
        setValue(res?.method);
        setIsVerified(true);
      }).catch(() => setValue("None"));
    }
  ...
  const onValueChange = ({
    ...
    if (value === "None" && isVerified) {
      ...
      dispatch(disableAuthenticationStatus()).then(unwrapResult).then(res => {
        setIsVerified(false);
  ...
  return <Fragment>
      ...
      <View style={styles.main}>
        ...
        <View style={styles.dropdownContainer} zIndex={1}>
          {!isVerified && !isLoading && <Text style={styles.authenticationText}>
              Two Factor Authentication is disabled, please enable it through
              one of the methods below
            </Text>}
          <DropDownPicker open={open} value={value} items={items} setOpen={setOpen} setValue={setValue} onSelectItem={onValueChange} />
        </View>

        {isVerified && <View style={[styles.wp90, styles.p5]}>

...
const Verification = ({
  ...
  const {
    ...
    link,
    authenticationType
  } = route.params; // This variables gets the loading status for sendVerification code api
  ...
  const verifyCodeLoading = useSelector(state => state?.Authentication?.verifyCode?.api?.loading); // This variables gets the loading status for enableAuthentication api
  ...
  return <Fragment>
      ...
      <View style={styles.mainVerificationView}>
        <View>
          {method === "google_authenticator" ? <Text style={styles.text}>
              ...
              {authenticationType && " Please enter it to enable two factor authentication"}
            </Text>}
          <Input label="Enter Code" returnKeyType="next" value={code} setValue={setCode} autoCapitalize="none" placeholder="Verification code" />
          <View>
            <Button mode="contained" onPress={handleVerification}>
              Verify
            </Button>
          </View>
          {method !== "google_authenticator" && <View style={styles.resend}>
        ...
        {method === "google_authenticator" && <View style={styles.pt15}>
